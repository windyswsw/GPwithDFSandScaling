...
void OldMutationForScale(Population* CurrentPop, FullSol* BaseFullSolution, CommonList* CurCommonListNew, short int FullSolSelected, int MutationPol, float NoOfServers, float NoOfLinks, short int MaxLinksCap, int w1, int w2, int w3, int w4, int w5){

	short int policyIndex = MutationPol;

	int PosOfChangingNF = 0;
	int NFSize = CurrentPop->CurPopulaton[FullSolSelected].VectorOfPartialSols[policyIndex].VMsForAPolicy.size()-1;

	if(CurrentPop->CurPopulaton[FullSolSelected].VectorOfPartialSols[policyIndex].VMsForAPolicy.size() > 1){
		PosOfChangingNF = rand() % NFSize +1;
	}
